...
Map<String, Integer> flatPrices = project.getFlatPrices();
project.getFlatTypes().forEach((type, count) -> {
    Integer price = flatPrices.get(type);
    String priceStr = (price != null) ? String.format("$%,d", price) : "N/A";
    System.out.println("  - " + type + ": " + count + " units, Price: " + priceStr);
});
