...
public class OfficerRegistrationController {
    ...
    /**
     * Checks if an officer is already registered for a project during the specified period.
     * 
     * @param officerNRIC The NRIC of the officer
     ...
     */
    // When checking if officer is already registered for an overlapping period
    private boolean isOfficerRegisteredDuringPeriod(String officerNRIC, Date startDate, Date endDate) {
        List<OfficerRegistration> registrations = officerRegistrationRepository.findByOfficer(officerNRIC);
        ...
        for (OfficerRegistration registration : registrations) {
            if (registration.isApproved()) {
                ...
                if (project != null) {
                    // Use DateUtils for overlap check
                    if (DateUtils.isDateRangeOverlapping(
                            startDate, endDate,
                            project.getOpeningDate(), project.getClosingDate())) {
                        return true;
                    }
